---
title: "A2UI Component Reference: Basic Catalog Gallery"
...
<Accordion title="Stack — layered overlay container">

Layers children on top of one another (z-axis stacking). Useful for overlays and badges.

<Tabs>
  <Tab title="v0.9">

    <ParamField body="children" type="array">
      Array of child component IDs, rendered in order from bottom to top.
    </ParamField>
    ...
    ```json
    {
      "id": "overlay-stack",
      "component": "Stack",
      "children": ["background-image", "overlay-text"]
    }
    ```
  ...
  <Tab title="v0.8">
    ...
    ```json
    {
      ...
      "component": {
        "Stack": {
          "children": { "explicitList": ["background-image", "overlay-text"] }
        }
      }
    }
    ```
...
<Accordion title="Padding — spacing wrapper">

Wraps a single child with configurable inset padding.
...
<Tabs>
  <Tab title="v0.9">
    ...
    <ParamField body="padding" type="number | object">
      Uniform padding value or `{ top, right, bottom, left }` object.
    </ParamField>
...
<Tabs>
  <Tab title="v0.9 — ChoicePicker">
    ...
    <ParamField body="selections" type="DataBinding" required>
      Path to the array of currently selected values in the data model.
    </ParamField>

    <ParamField body="maxAllowedSelections" type="number">
      Maximum number of simultaneous selections. Use `1` for a single-select (dropdown equivalent).
    </ParamField>
  ...
  <Tab title="v0.8 — MultipleChoice">
    ...
    In v0.8, option labels use the `literalString` wrapper.
